...
if nargin < 3 || isempty(D) || ~iscell(D)
...
%   Get wavelet
LPD = zeros([size(Faf{1},1),2,length(D)],class(img));
HPD = zeros([size(Faf{1},1),2,length(D)],class(img));
for ri = 1:2
    LPD(:,ri,1) = Faf{ri}(:,1);
    HPD(:,ri,1) = Faf{ri}(:,2);
    if length(D) > 1
...
if isnumeric(D)
    if length(D)==1 % include this case explicitly for improved 1-D speed
        [WC{1}.A, WC{1}.D, WC{2}.A,WC{2}.D] = fwt1dMEX(img,LPD(:,1),HPD(:,1),D);
        WC{1}.odd_dims = isodd(size(img));
        WC{2}.odd_dims = isodd(size(img));
    else
        [WC{1}, WC{2}] = fwtn_sub(img,LPD(:,1),HPD(:,1),D);
    end
    ...
    for ii = 1:ND
        ...
        [WC{1,ii} ] = fwtn_sub(img,LPD(:,ii),HPD(:,ii),D{ii});
