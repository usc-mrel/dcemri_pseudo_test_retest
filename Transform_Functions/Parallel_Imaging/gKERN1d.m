function G = gKERN1d(kcal,opt)
%   Computes the 2D GRAPPA/SPIRiT kernel
%   
%   Author: RML
%   Date: 02/2011
%   
%   Usage: G = gKERN1d(kcal,opt)
%   
%   Input:
...
Gt = G * opt.size(2);
G = zeros([1 opt.size(2) opt.size(3) 1 nr nr],opt.class);
G(1,ceil((opt.size(2)-kf(2))/2)+1:ceil((opt.size(2)+kf(2))/2),:,:,:,:) = Gt;
clear Gt
opt.FTshift = 1;
G = opt.iFT(G,opt);
G = repmat(G,[opt.size(1) 1 1 1 1]);
