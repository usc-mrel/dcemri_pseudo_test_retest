...
void mexFunction(int nlhs, mxArray *left[], int nrhs, const mxArray *right[]) {
    ...
    if (mxIsComplex(right[0]) != mxIsComplex(right[1]))
        mexErrMsgTxt("iGRAPPA_MEX: Inputs real/complex mismatch");
    if (mxGetClassID(right[0]) != mxGetClassID(right[1]))
        mexErrMsgTxt("iGRAPPA_MEX: Input data type mismatch");
		...
		for (iNRS=0; iNRS<sz[4]; iNRS++) {
            ...
            for (iNR=0; iNR<sz[4]; iNR++) {
                ...
                for (iNV=0; iNV<sz[1]; iNV++) {
                    ...
                    indGt  = iNV*s0 + iNS*s1 + iNR*s2 + iNRS*s2*sz[4];
                    for (iNP=0; iNP<sz[0]; iNP++) {
                        indKG = iNP + indKGt;
                        indK  = iNP + indKt;
                        indG  = iNP + indGt;
                        pKGr[indKG] += pKr[indK]*pGr[indG] + pKi[indK]*pGi[indG];
                        pKGi[indKG] += pKi[indK]*pGr[indG] - pKr[indK]*pGi[indG];
                    }
                }
                }
                }
            ...
            for (iNR=0; iNR<sz[4]; iNR++) {
                ...
                for (iNV=0; iNV<sz[1]; iNV++) {
                    ...
                    for (iNP=0; iNP<sz[0]; iNP++) {
                        ...
                        pKGif[indKG] += pKif[indK]*pGrf[indG] - pKrf[indK]*pGif[indG];
                    }
                }
                }
                }
	...
	if (precision == mxDOUBLE_CLASS) {
        #pragma omp parallel for private(iNR,iNT,iNS,iNV,iNP,indK,indKt)
		for (iNR=0; iNR<sz[4]; iNR++) {
			for (iNT=0; iNT<sz[3]; iNT++) {
            for (iNS=0; iNS<sz[2]; iNS++) {
            for (iNV=0; iNV<sz[1]; iNV++) {
                /*indKt = iNV*s0 + iNS*s1 + iNT*s2  + iNR*s3;*/
                for (iNP=0; iNP<sz[0]; iNP++) {
                    /*indK = iNP + indKt; */
