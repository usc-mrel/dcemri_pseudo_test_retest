...
%   modified by Yannick Bliesener 2017-01-10
%
...
weightedDescent = false;
projectedDescent = false;

if isfield(options,'descentWeights')
    % applies weights to the gradient
    weightedDescent = true;
    descentWeights = options.descentWeights;
end

if isfield(options,'funProj')
    % projector function for projected gradient descent
    projectedDescent = true;
    funProj = options.funProj;
end
...
for i = 1:maxIter
    ...
    elseif LS_type == 1 % Find Point satisfying Wolfe conditions
        ...
        if computeHessian
            [t,f,g,LSfunEvals,LSiter,H] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,maxLSiter,progTol,debug,doPlot,LS_saveHessianComp,funObj,varargin{:});
        else
            [t,f,g,LSfunEvals,LSiter] = WolfeLineSearch(x,t,d,f,g,gtd,c1,c2,LS_interp,LS_multi,maxLSiter,progTol,debug,doPlot,1,funObj,varargin{:});
        end
        funEvals = funEvals + LSfunEvals;
    ...
    if verbose
        ...
        fprintf('------------------------\n');
        fprintf('Stepsize: %.3e\n', t);
        fprintf('Iteration line search: %i\n', LSiter);
        fprintf('Function value: %.3e\n', f);
        fprintf('Gradient norm: %.3e\n', norm(g)); 
        fprintf('Search direction norm: %.3e\n', norm(d));
...
%        imagesc(real(reshape(x,[256 150])),[0 0.3]);axis image; axis off; drawnow;
    ...
    if maxLSiter == LSiter
        exitflag=2;
        msg = 'Line search reached maximum number of iterations!';
        break;
    end
