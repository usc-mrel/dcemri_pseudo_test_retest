function [img,opt] = SPSENSE_recon(k,opt)
...
%   It also resets the conjugate gradient direction
    function PeriodicUpdate
        ...
        if any(opt.compression_levels > 0)
            updt = 0;
            ...
            for i = find(opt.lambda>0)
                ...
                switch i
                    case 2
                        CF = abs(CELLextract(WC,7));
                    case 3
                        CF = abs(CELLextract(TV1,7));
                    case 4
                        CF = abs(CELLextract(TV2,7));
                    case 5
                        CF = abs(CELLextract(VS,7));
                    case 6
                        CF = abs(CELLextract(RI,7));
                    case 7
                        CF = abs(CELLextract(CD,7));
                end
                CL = 100*sum(CF<th/2)/numel(CF);
                fprintf('Compression level (%d): %5.1f (target = %5.1f)',i,CL,opt.compression_levels(i));
                
                %   Check if current compression level is close to target
                fprintf('; Constraint: %7.6g',opt.lambda(i));
                if abs(CL - opt.compression_levels(i)) > 1
                    opt.lambda(i) = mean([opt.lambda(i) 2*prctile(CF,opt.compression_levels(i))]);
                    updt = 1;
                    grad_restart_flag = 1;
                    fprintf(' -> %7.6g',opt.lambda(i));
                end
                CF = [];
                fprintf('\n');
            end
            
            %   Execute only if constraints have been changed
%             if updt
%                 %   Update phase estimate
%                 if opt.phase_corr
%                     comp_Ph;
%                     if opt.verbose == 2
%                         fprintf('Phase estimate updated\n');
%                     end
%                 end
%                 
%                 %   Reset delta (conjugate gradient restart)
%                 %   Whe lambda has been modifed, the objective function has
%                 %   been changed; the previous gradient history no longer
%                 %   applies
%                 grad_restart_flag = 1;
%             end
            
            %   Reset search direction if negative improvement
%             if (opt.RI.Iter > opt.update) && any(diff(opt.RI.Obj(end-opt.update+1:end)) > 0)
%                 grad_restart_flag = 1;
%             end
            
        end
        
        %   Compute new l1weights
%         if opt.l1weight > 0
%             opt.l1weight = 0.5*(1-exp(-opt.RI.Iter/25));
%             if opt.verbose
%                 fprintf('Updated weights for l1-norms: %d\n',opt.l1weight);
%             end
%             comp_l1weight;
%             grad_restart_flag = 1;
%         end
...
%   Check options structure against input data and disable certain options if warranted
    function param_check
        ...
        if nt ==1 && opt.lambda(6)
            opt.lambda(6) = 0;
            if opt.verbose
                fprintf('Dynamic reference image  disabled (only one time point)\n');
            end
        end
