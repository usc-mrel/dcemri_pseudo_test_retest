function [ kt, vp, ve, flag, res, iter ] = conc2tk_etofts_gpu(N, Nt, conc, modelParam, opt)
%function [ kt, vp, ve, flag, res, iter ] = conc2tk_etofts_gpu(N, Nt, conc, modelParam, opt)
%   fits the PK parameters of the extended Tofts model parameterized with ve
%
%   Inputs
%       N                   number of voxels
...
if isfield(opt, 'constraints')
    constraints = [opt.constraints.kt(:) * 60; opt.constraints.ve(:); opt.constraints.vp(:)];
...
% initial parameters
if isfield(opt, 'init')
    initial_parameters = cat(1, reshape(opt.init.kt, 1, N), reshape(opt.init.ve, 1, N), reshape(opt.init.vp, 1, N));
