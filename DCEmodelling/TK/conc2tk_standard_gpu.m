function [ kt, vp, kep, flag, res, iter ] = conc2tk_standard_gpu(N, Nt, conc, modelParam, opt)
%function [ kt, vp, kep, flag, res, iter ] = conc2tk_standard_gpu(N, Nt, conc, modelParam, opt)
%   fits the PK parameters of the extended Tofts model parameterized with Kep instead of ve
%
%   Inputs
%       N                   number of voxels
...
if isfield(opt, 'constraints')
    constraints = [opt.constraints.kt(:) * 60; opt.constraints.kep(:) * 60; opt.constraints.vp(:)];
...
model_id = ModelID.DCE_STANDARD;

% initial parameters
if isfield(opt, 'init')
    initial_parameters = cat(1, reshape(opt.init.kt * 60, 1, N), reshape(opt.init.kep * 60, 1, N), reshape(opt.init.vp, 1, N));
